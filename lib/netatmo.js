      ...
      if (!_.isArray(home.modules)) {
        ...
        if (this.houses.find(l => l.homeid == home.id)) {
          ...
          for (let m = 0, mlen = home.modules.length; m < mlen; m++) {
            ...
            // Add ID and NAME in modinfos & mod_name
            mod_name.set(infos.id, infos.name)
        ...
        log.debug('Get home discovery for ' + home.id)
        params = {
          home_id: home.id
        }
        ...
        for (const module of status.modules) {
          ...
          const data = {}
          data.id = module.id
          data.name = mod_name.get(module.id)
